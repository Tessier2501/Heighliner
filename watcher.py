...
假设目录结构 (脚本所在目录):
    ...
    2_processed/
    3_failed/
    rename_invoice.py ← 你现有的解析/重命名脚本, 需与本脚本同目录

说明:
- 只处理后缀严格为 .pdf 的文件 (避免下载中被处理)
- 要求文件名 (不含扩展) 为纯数字且长度在 10~20 之间 (可根据需要调整)
- 本脚本不会改变你 rename_invoice.py 的内部逻辑, 只设置其 PLACEHOLDER_PREFIX 并调用 process_pdf()
"""
...
def find_renamed_output_for_order(order_id: str) -> list[Path]:
    ...
    这是基于你 rename_invoice.py 中使用 PLACEHOLDER_PREFIX 来生成文件名的约定.
    """
    ...
    while True:
        try:
            ...
            for incoming_file in pdfs:
                ...
                # 将订单号设置为 rename_invoice 中的前缀（你的脚本会用它来命名输出文件）
                try:
                    renamer.PLACEHOLDER_PREFIX = order_id
                except Exception:
                    # 如果 rename_invoice 没有该变量，也无妨，我们仍然调用 process_pdf（但按你的脚本需要这个变量）
                    print("  WARNING: couldn't set PLACEHOLDER_PREFIX on rename_invoice module. Continuing...")

                # 调用你的现有处理函数
                try:
                    print(f"  Processing (order={order_id}) ...")
                    # 你的 process_pdf 以 pdf_path 为参数并负责重命名原始文件到 <order>_<amount>_<invoice>.pdf
